...
- **WHEN** the user runs `heimdall-sandbox exec --policy <file>` with a JSON policy containing cwd, command, stdio, and shared sandbox config fields (`enabled`, `network`, `paths`, and `env`)
...
- **WHEN** a JSON policy contains `enabled: true`, `network: "host"`, `env.allow`, and `env.deny`
- **THEN** the CLI parses those fields using the shared sandbox config shape

#### Scenario: JSON policy rejects unsupported network isolation
- **WHEN** a JSON policy contains `network: "none"`
- **THEN** the system exits with a sandbox misconfiguration error until platform network isolation is implemented

#### Scenario: JSON policy rejects unsupported path isolation
- **WHEN** a JSON policy contains non-empty `paths`
- **THEN** the system exits with a sandbox misconfiguration error until filesystem isolation is implemented
