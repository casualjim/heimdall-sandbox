...
pub fn execute(request: &ExecRequest) -> Result<i32> {
    execute_with_hardener(request, heimdall_process_hardening::apply_child_hardening)
}
...
fn execute_with_hardener(
    ...
    let child_environment = strip_dangerous_environment(child_environment);

    #[cfg(unix)]
    let forwarding = crate::signal::SignalForwarding::install()?;
    ...
    configure_stdio(&mut command, request.stdio_policy());
    ...
    let output_forwarding = OutputForwarding::start(&mut child, request.stdio_policy());
...
mod tests {
    ...
    fn hardening_failure_maps_to_misconfiguration_exit_code() {
        ...
        let error = execute_with_hardener(&request, || Err(io::Error::other("hardening failed")))
